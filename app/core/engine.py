...
async def translate_in_node(state: PipelineState):
    ...
    if lang.lower() in ['en','english']:
        ...
        # log_state = {**state, "english_question": text, "latencies": new_latencies}
        # log_step_to_csv(log_state, "translate_in", "N/A (English Detected)", text)
        return {
    ...
    #     eng_text = result.get("english_translation", text) 
    # except Exception as e:
    #     print(f"[ERROR] LLM Language Detection Failed: {e}. Fallback to English.")
    #     detected_lang = "English"
    #     eng_text = text      
    
    # log_state = {**state, "english_question": translated_q, "latencies": new_latencies}
    # log_step_to_csv(log_state, "translate_in", prompt, translated_q)
    new_latencies = {**state.get("latencies",{}), "translate_in": time.time() - start}
    return {
...
async def translate_out_node(state: PipelineState):
    ...
    if lang.lower() in ['en','english']:
        ...
        # Log bypassed translation
        # log_state = {**state, "latencies": new_latencies}
        # log_step_to_csv(log_state, "translate_out", "N/A (English Detected)", state['response'])
    ...
    # log_state = {**state, "response": translated_res, "english_response": state['response'], "latencies": new_latencies}
    # log_step_to_csv(log_state, "translate_out", prompt, translated_res)
